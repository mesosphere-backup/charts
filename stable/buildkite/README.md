...
```bash
$ helm install dlc/buildkite --name bk-agent --namespace buildkite --set agent.token="BUILDKITE_AGENT_TOKEN"
```

To install the chart with the release name `bk-agent` and set Agent meta-data and git repo SSH key:
```console
$ helm install dlc/buildkite --name bk-agent --namespace buildkite \
  --set agent.token="$(cat buildkite.token)",agent.meta="role=production" \
  --set privateSshKey="$(cat buildkite.key)",registryCreds.dockerConfig="$(cat docker.json | base64)"
```
...
registryCreds.dockerConfig: docker registry credentials read from file and base64 encoded
```
